 ...
 */
fun Modifier.scaleClickable(pressedScale: Float = 0.95f, onClick: () -> Unit): Modifier = composed {
 ...
 */
fun Modifier.scaleOnPress(pressedScale: Float = 0.90f): Modifier = composed {
    var isPressed by remember { mutableStateOf(false) }
    val scale by
            animateFloatAsState(
                    targetValue = if (isPressed) pressedScale else 1f,
