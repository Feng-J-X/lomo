...
class MemoTextProcessor @Inject constructor() {

    companion object {
        // Permissive regex: Matches HH:mm OR HH:mm:ss
        private val MEMO_BLOCK_END = Regex("^-\\s\\d{2}:\\d{2}(?::\\d{2})?.*")
        private val TAG_PATTERN =
                java.util.regex.Pattern.compile("(?:^|\\s)#([\\p{L}\\p{N}_][\\p{L}\\p{N}_/]*)")
        private val MD_IMAGE_PATTERN = java.util.regex.Pattern.compile("!\\[.*?\\]\\((.*?)\\)")
        private val WIKI_IMAGE_PATTERN = java.util.regex.Pattern.compile("!\\[\\[(.*?)\\]\\]")
    }
     ...
     */
    fun findMemoBlock(lines: List<String>, rawContent: String, timestamp: Long): Pair<Int, Int> {
    ...
    fun removeMemoBlock(lines: MutableList<String>, rawContent: String, timestamp: Long): Boolean {
    ...
    fun toggleCheckbox(content: String, lineIndex: Int, checked: Boolean): String {
