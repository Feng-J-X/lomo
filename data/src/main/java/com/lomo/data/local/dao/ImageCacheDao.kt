...
interface ImageCacheDao {
    @Query("SELECT * FROM image_cache") fun getAllImages(): Flow<List<ImageCacheEntity>>

    @Query("SELECT * FROM image_cache") suspend fun getAllImagesSync(): List<ImageCacheEntity>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertImages(images: List<ImageCacheEntity>)

    @Query("DELETE FROM image_cache") suspend fun clearAll()
