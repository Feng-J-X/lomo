...
interface MemoDao {
        ...
        @Query("DELETE FROM lomo_fts") suspend fun clearFts()

        @Insert(onConflict = OnConflictStrategy.REPLACE)
        suspend fun insertMemos(memos: List<MemoEntity>)

        @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertMemo(memo: MemoEntity)
        ...
        @Query("DELETE FROM Lomo") suspend fun clearAll()

        @Query("DELETE FROM Lomo WHERE id NOT IN (:ids)")
        suspend fun deleteMemosNotIn(ids: List<String>)

        @Query("SELECT * FROM Lomo WHERE id = :id") suspend fun getMemo(id: String): MemoEntity?

        @Query("SELECT * FROM Lomo") suspend fun getAllMemosSync(): List<MemoEntity>

        @Query("SELECT * FROM Lomo WHERE date = :date AND isDeleted = :isDeleted")
        suspend fun getMemosByDate(date: String, isDeleted: Boolean): List<MemoEntity>
    ...
    """
        ...
        @Query("SELECT * FROM tags") fun getAllTags(): Flow<List<TagEntity>>

        // Stats
        @Query("SELECT COUNT(*) FROM Lomo WHERE isDeleted = 0") fun getMemoCount(): Flow<Int>
        ...
        suspend fun countMemosWithImage(imagePath: String, excludeId: String): Int
        ...
        @Query("DELETE FROM memo_tag_cross_ref") suspend fun clearCrossRefs()
}
