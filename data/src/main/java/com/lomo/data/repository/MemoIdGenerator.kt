 ...
 */
@Singleton
class MemoIdGenerator @Inject constructor(
    ...
    suspend fun generateUniqueId(timestamp: Long): Pair<Long, String> {
        ...
        while (retryCount < MAX_RETRIES) {
            ...
            val filename = DateTimeFormatter.ofPattern(filenameFormat)
                .withZone(zoneId)
                .format(instant) + ".md"
            
            val timeString = DateTimeFormatter.ofPattern(timestampFormat)
                .withZone(zoneId)
                .format(instant)
    ...
    suspend fun generateFilename(timestamp: Long): String {
        val filenameFormat = dataStore.storageFilenameFormat.first()
        return DateTimeFormatter.ofPattern(filenameFormat)
            .withZone(ZoneId.systemDefault())
